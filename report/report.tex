...
\begin{itemize}[leftmargin=*, label=\textcolor{primary}{\faCog}]
    ...
    \item \textbf{Tracking:} All runs logged to MLflow with hyperparameters, metrics, and model artifacts
\end{itemize}
...
The pipeline (SMOTE + LightGBM) was saved as \tech{best\_model.joblib}, registered in the MLflow Model Registry, and published to Hugging Face Hub.
\end{infobox}
...
\midrule
Ahmed ElSayed & Exploratory Data Analysis notebook, data cleaning pipeline (\tech{data\_cleaning.py}), feature engineering (\tech{feature\_engineering.py}), ML model training (4 models, stratified CV), MLflow experiment tracking, SHAP explainability \\
\midrule
